<template>
  <div class="pagination">
    ...
    <!-- 数字也是可以循环创建的 -->
    <button v-for="(page,index) in startEnd.end" :key="index"
    v-if="page > startEnd.start" :class="{active:page === currentPageNum}"
    @click="$emit('changePage',page)">{{page}}</button>
    <button v-show="startEnd.end < totalPage - 1">···</button>
    <button v-show="startEnd.end < totalPage" :class="{active:currentPageNum === totalPage}"
    @click="$emit('changePage',totalPage)">{{totalPage}}</button>
    <button :disabled="currentPageNum === totalPage" @click="$emit('changPage',currentPageNum + 1)">下一页</button>
